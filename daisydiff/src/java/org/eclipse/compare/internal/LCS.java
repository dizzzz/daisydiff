...
public abstract class LCS {
	...
	if ((double) length1 * (double) length2 > settings.getTooLong()) {
	    ...
	    max_differences = (int) Math.pow(max_differences, settings.getPowLimit() - 1.0);
	}
    ...
    /**
         ...
         * @param V
         *                should be allocated as int[2][l1.length + l2.length +
         *                1], used to store furthest reaching D-paths
         * @param snake
         *                should be allocated as int[3], used to store the
         *                beginning x, y coordinates and the length of the
         *                latest snake traversed
         * @param subMonitor
         * @param lcs
         *                should be allocated as TextLine[2][l1.length], used to
         *                store the common points found to be part of the LCS
         *                where lcs[0] references lines of l1 and lcs[1]
         *                references lines of l2.
         * 
	...
	if (d > 1) {
	    ...
	    /*
                 ...
                 * already saved all the lines after the difference in the for
                 * loop above, now we need to save all the lines before the
                 * difference.
                 */
	...
	snake[2] = 0;
	return 5; /*
                         * HACK: since we didn't really finish the LCS
                         * computation we don't really know the length of the
                         * SES. We don't do anything with the result anyway,
                         * unless it's <=1. We know for a fact SES > 1 so 5 is
                         * as good a number as any to return here
                         */
    }
    ...
    /**
         ...
         * @param V
         *                the array storing the furthest reaching D-paths for
         *                the LCS computation
         * @return The result as an array of 3 integers where result[0] is the x
         *         coordinate of the current location in the diagonal with the
         *         most progress, result[1] is the y coordinate of the current
         *         location in the diagonal with the most progress and result[2]
         *         is the amount of progress made in that diagonal
         */
