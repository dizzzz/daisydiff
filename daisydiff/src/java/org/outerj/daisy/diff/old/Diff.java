...
public class Diff {
    ...
    /**
     * Diffs two texts, outputting the result to the specified DiffOutput
     * instance.
     * 
     * @param contextLineCount
     *            specify -1 to include complete text
     * @throws Exception
     *             the differencing itself should normally not throw exceptions,
     *             but the methods on DiffOutput can.
     */
    public static void diff(String text1, String text2, DiffOutput output,
            int contextLineCount) throws Exception {
        ...
        if (differences.length > 0) {
            ...
            while (diffIndex < differences.length && pos < leftLineCount) {
                ...
                if (diff.kind() == RangeDifference.CHANGE) {
                    ...
                    // output contextLineCount number of lines (if available) or
                    // all lines if contextLineCount == -1
                    if (pos != 0) { // at start of file, skip immediately to
                                    // first changes
                        int beginContextEndPos = pos + contextLineCount;
                        while ((pos < beginContextEndPos || contextLineCount == -1)
                                && pos < nextChangedLine) {
                            output.startLine(DiffLineType.UNCHANGED);
                            output
                                    .addUnchangedText(leftComparator
                                            .getLine(pos));
                            output.endLine();
                            pos++;
                        }
                    ...
                    if (contextLineCount >= 0) {
                        int endContextStartPos = nextChangedLine
                                - contextLineCount;
                        if (endContextStartPos > pos + 1) { // the +1 is to
                                                            // avoid skipping
                                                            // just one line
                    ...
                    if (diff.leftLength() > 0 && diff.rightLength() > 0) {
                        leftBlock = concatLines(leftComparator, diff
                                .leftStart(), diff.leftLength());
                        rightBlock = concatLines(rightComparator, diff
                                .rightStart(), diff.rightLength());
                    }
                    ...
                    if (leftBlock == null) {
                        for (int i = 0; i < diff.leftLength(); i++) {
                            ...
                            output.addUnchangedText(leftComparator
                                    .getLine(currentLine));
                            output.endLine();
                        }
                    } else {
                        diffBlock(leftBlock, rightBlock, output,
                    ...
                    if (leftBlock == null) {
                        for (int i = 0; i < diff.rightLength(); i++) {
                            ...
                            output.addUnchangedText(rightComparator
                                    .getLine(currentLine));
                            output.endLine();
                        }
                    } else {
                        diffBlock(rightBlock, leftBlock, output,
                                DiffLineType.ADDED);
                    }
                }
    ...
    private static StringBuilder concatLines(TextComparator comparator,
            int start, int count) {
    ...
    private static void diffBlock(StringBuilder block1, StringBuilder block2,
            DiffOutput output, DiffLineType diffLineType) throws Exception {
        ...
        RangeDifference[] lineDiffs = RangeDifferencer.findDifferences(
                leftBlockComparator, rightBlockComparator);
        ...
        for (int i = 0; i < lineDiffs.length; i++) {
            ...
            if (pos < left) {
                String[] strings = leftBlockComparator.substringSplitted(pos,
                        left);
            ...
            if (diff.leftLength() > 0) {
                String[] strings = leftBlockComparator.substringSplitted(left,
                        diff.leftEnd());
                for (int d = 0; d < strings.length; d++) {
                    if (strings[d].equals("\n")) {
                        output.endLine();
        ...
        if (diff == null
                || diff.leftEnd() < leftBlockComparator.getRangeCount()) {
            int start = 0;
            if (diff != null)
                start = diff.leftEnd();
