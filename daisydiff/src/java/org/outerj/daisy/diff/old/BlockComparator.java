...
/**
 * A Comparator for diffing corresponding changes resulting from a line-based
 * diff. For example, if a line-based diff says "these lines were replaced by
 * those lines", then this comparator can be used to compare the content in
 * those lines.
 * 
 * <p>
...
public class BlockComparator implements IRangeComparator {
     ...
     * corresponding to one line. The newlines themselves are also entries in
     * the array.
     */
    ...
    private ArrayList splitLineTokens(StringBuilder text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            switch (c) {
            case '<': // begin of a HTML/XML tag: let it stick to the next
                        // word
            ...
            case '>':
                if (currentWord.length() > 2 && currentWord.charAt(0) == '<'
                        && currentWord.charAt(1) == '/') {
                    currentWord.append(c);
                    break;
                }
            case '.':
